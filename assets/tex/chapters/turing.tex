...
\section{Random Walks Model Diffusion}
...
Turing's insight was that high-level patterns could arise in certain circumstances if we combine particle diffusion with chemical reactions, in which colliding particles interact. Such a system is called a \textdef{reaction-diffusion system}{reaction-diffusion system}{a system incorporating the diffusion of particles with chemical reactions involving these particles}, and the emergent patterns are called \textdef{Turing patterns}{Turing patterns}{patterns, such as stripes and spots, that emerge from certain reaction-diffusion systems} in his honor.

We will consider a reaction-diffusion system having two types of particles, \textvar{A} and \textvar{B}. The system is not explicitly a predator-prey relationship, but you may like to think of the \textvar{A} particles as prey and the \textvar{B} particles as predators.

Both types of particles diffuse randomly through the plane, but the \textvar{A} particles diffuse more quickly than the \textvar{B} particles.  In what follows, we will assume that \textvar{A} particles diffuse twice as quickly as \textvar{B} particles. In terms of our random walk model, this faster rate of diffusion means that in a single time ``step'', an \textvar{A} particle moves twice as far as a \textvar{B} particle.\\
...
\noindent To simulate this reaction on a particle level, if an \textvar{A} particle and two \textvar{B} particles collide with each other, then the \textvar{A} particle has some fixed probability of being replaced by a third \textvar{B} particle, which could vary in practice based on the presence of a catalyst and the orientation of the particles when they collide. This probability directly relates to the \textit{rate} at which the reaction occurs, denoted \textvar{r}. This third reaction is why we compared \textvar{A} to prey and \textvar{B} to predators, since we may like to conceptualize the reaction as two \textvar{B} particles consuming an \textvar{A} particle and producing a new offspring \textvar{B} particle.\\
...
Four parameters are relevant to our reaction-diffusion system: the feed rate (\,\textvar{f}) of \textvar{A} particles, the kill rate (\textvar{k}) of \textvar{B} particles, the predator-prey reaction rate (\textvar{r}), and the diffusion rate (i.e., speed) of \textvar{B} particles. We do not need to set the diffusion rate of \textvar{A} particles, since we know that their diffusion is twice that of \textvar{B} particles.
...
We will now initiate our reaction-diffusion system with a uniform concentration of \textvar{A} particles spread across a two-dimensional plane and a tightly packed collection of \textvar{B} particles in the center of the plane. As \textvar{B} particles diffuse, what will we see?\tutorial[prologue/turing-cellblender]\\

\FloatBarrier
\phantomsection
\subsection{Changing parameters yields different Turing patterns}

For many parameter values, our reaction-diffusion system is not very interesting. \autoref{fig:k=500000_f=1000} shows that if the kill rate is too high, then \textvar{B} particles (colored red) will die out more quickly than they can be replenished by the reaction with \textvar{A} particles (colored green), and so only \textvar{A} particles will remain.

\begin{figure}[h]
\centering
\mySfFamily
\begin{tabular}{c c c c}
\includegraphics[width = 0.19\textwidth]{../images_CMYK/predator_prey_predator_dies_f1e1_r5e5} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_predator_dies_f1e1_r5e5_i1} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_predator_dies_f1e1_r5e5_i2} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_predator_dies_f1e1_r5e5_i3}\\[2ex]
...
\end{tabular}
\caption{An eight-panel animation of a simulation of our reaction-diffusion system using the parameter rates \textvar{f} = 1,000 and \textvar{k} = 500,000. As the animation proceeds (from top left to bottom right), small pockets of \textvar{B} particles form, but they die out more quickly than they can be replenished, leaving only \textvar{A} particles.}
\label{fig:k=500000_f=1000}
\end{figure}

Conversely, \autoref{fig:k=100000_f=1000000} shows that if \textvar{f} is too high, then the increased feed rate will cause an increase in the concentration of \textvar{A} particles. However, the increased concentration of \textvar{A} particles will also lead to more collisions between \textvar{A} particles and pairs of \textvar{B} particles, causing the concentration of \textvar{B} particles to explode.\\

\begin{figure}[h]
\centering
\mySfFamily
\begin{tabular}{c c c c}
\includegraphics[width = 0.19\textwidth]{../images_CMYK/predator_prey_f1e6_d1e5} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_f1e6_d1e5_i1} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_f1e6_d1e5_i2} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_f1e6_d1e5_i3}\\[2ex]
\includegraphics[width = 0.19\textwidth]{../images_CMYK/predator_prey_f1e6_d1e5_i4} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_f1e6_d1e5_i5} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_f1e6_d1e5_i6} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_f1e6_d1e5_i7}
\end{tabular}
\caption{An animation of a simulation of our reaction-diffusion system using the parameter rates \textvar{f} = 1,000,000 and \textvar{k} = 100,000. Because of the high feed rate and low kill rate, the ``predator'' \textvar{B} particles expand rapidly and achieve higher concentration than the \textvar{A} particles across the system.}
\label{fig:k=100000_f=1000000}
\end{figure}

The interesting behavior in this system lies in a sweet spot of \textvar{f} and \textvar{k} parameter values. For such values, \autoref{fig:k=200000_f=100000} shows a clear stripe of \textvar{B} particles expanding outward against a background of \textvar{A} particles, with subsequent stripes appearing at locations where a critical mass of \textvar{B} particles can interact with each other.
...
As \textvar{f} continues to increase, the stripe structure becomes chaotic and breaks down because many clusters of \textvar{B} particles are now colliding and mixing (\autoref{fig:k=200000_f=175000}).

A subsequent increase in \textvar{f} causes the stripes to disappear (\autoref{fig:k=100000_f=100000}). We might expect this to mean that the \textvar{A} and \textvar{B} particles will be uniformly mixed. Instead, after an initial outward explosion of \textvar{B} particles, the system displays a \textit{mottled} background.

\begin{figure}[h]
\centering
\mySfFamily
\begin{tabular}{c c c c}
\includegraphics[width = 0.19\textwidth]{../images_CMYK/predator_prey_11_by_11_f_1_k_2} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_2_i1} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_2_i2} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_2_i3}\\[2ex]
\includegraphics[width = 0.19\textwidth]{../images_CMYK/predator_prey_11_by_11_f_1_k_2_i4} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_2_i5} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_2_i6} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_2_i7}
\end{tabular}
\caption{An animation of a simulation of our reaction-diffusion system using the parameter rates \textvar{f} = 100,000 and \textvar{k} = 200,000. For these ``sweet spot'' parameters, we see expanding stripes of \textvar{B} particles.}
...
\end{tabular}
\caption{An animation of a simulation of our reaction-diffusion system using the parameter rates \textvar{f} = 140,000 and \textvar{k} = 200,000. Because \textvar{f} has increased relative to \textvar{k} compared to \autoref{fig:k=200000_f=100000}, we see even more stripes of \textvar{B} particles.}
...
\mySfFamily
\begin{tabular}{c c c c}
\includegraphics[width = 0.19\textwidth]{../images_CMYK/predator_prey_11_by_11_f_1.75_k_2_new} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1.75_k_2_new_i1} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1.75_k_2_new_i2} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1.75_k_2_new_i3}\\[2ex]
...
\end{tabular}
\caption{An animation of a simulation of our reaction-diffusion system using the parameter rates \textvar{f} = 175,000 and \textvar{k} = 200,000. So many pockets of \textvar{B} particles have arisen that the stripe patterns start to break down.}
...
\mySfFamily
\begin{tabular}{c c c c}
\includegraphics[width = 0.19\textwidth]{../images_CMYK/predator_prey_11_by_11_f_1_k_1} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_1_i1} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_1_i2} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_1_i3}\\[2ex]
\includegraphics[width = 0.19\textwidth]{../images_CMYK/predator_prey_11_by_11_f_1_k_1_i4} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_1_i5} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_1_i6} & \includegraphics[width = 0.19\textwidth]{../images_CMYK/../images_CMYK/predator_prey_11_by_11_f_1_k_1_i7}
\end{tabular}
\caption{An animation of a simulation of our reaction-diffusion system using the parameter rates \textvar{f} = 100,000 and \textvar{k} = 100,000. Now that the feed rate is equal to the kill rate, after an initial wave of \textvar{B} particles, the \textvar{B} particles form so many clusters that they produce a mottling effect.}
...
Over five decades after Klüver's work, researchers would determine that form constants originate from simpler linear \textit{stripes} of cellular activation patterns in the retina. When the brain passes these linear patterns from the circular retina to the rectangular field of view interpreted by the visual cortex, it contorts these stripes into form constants. Some researchers believe that the patterns in the retina caused by hallucinations are in fact Turing patterns and can be explained by a reaction-diffusion model in which one type of neuron acts as a predator and another acts as prey, but this hypothesis remains unresolved.\\
...
\end{tabular}
\caption{(Left) A $\text{5} \times \text{5}$ grid showing hypothetical initial concentrations of \textvar{A} particles. Cells are labeled by decimal numbers representing their concentration of \textvar{A} particles. The central cell has maximum concentration, and no particles are contained in any other cell. (Right) An updated grid following diffusion of particles after a single time step.}
...
\begin{qbox}[%
What should the values of the ``?'' cells be in \autoref{fig:A_concentration_two_time_steps_partial}?
]\end{qbox}
...
Our solution is to slow down the diffusion process by adding a parameter $d_A$ having values between 0 and 1 that represents the \textit{rate} of diffusion of \textvar{A} particles. Instead of moving a cell's entire concentration of particles to its neighbors in a single time step, we move only a fraction $d_A$ of them.
...
\autoref{fig:diffusion_movie_still} shows an animation of a $101 \times 101$ board with $d_A = 0.5$ and $d_A = 0.25$ that begins with $[\textvar{A}] = 1$ for all cells. All cells have $[\textvar{B}] = 0$ except for an $11 \times 11$ square in the middle of the grid, where $[\textvar{B}] = 1$. (There is nothing special about the dimensions of this central square.) Without looking at individual concentration values, this animation allows us to see immediately that the \textvar{A} particles are remaining in the corners, while a band of \textvar{B} particles expands outward from the center.\\

\begin{note}[%
Particles technically ``fall off'' the sides of the board in \autoref{fig:diffusion_movie_still}, meaning that cells on the boundary lose some of their particle concentration that is not gained by their neighbors.
]\end{note}
...
\end{tabular}
\caption{A collection of still images from an animation of a two-particle diffusion automaton in which a central square of \textvar{B} particles expand outward within a uniform field of \textvar{A} particles.}
...
First, we have the feed reaction, which takes place at a rate \textvar{f}. It is tempting to simply add some constant value \textvar{f} to the concentration of \textvar{A} in each cell in each time step. However, if $[\textvar{A}]$ were close to 1, then adding \textvar{f} to it could cause $[\textvar{A}]$ to exceed 1, which we may wish to avoid.

Instead, if a cell has current concentration $[\textvar{A}]$, then we will add $\textvar{f} \cdot (1-[\textvar{A}])$ to this cell's concentration of \textvar{A} particles. For example, if $[\textvar{A}]$ is equal to 0.01, then we will add $0.99 \cdot \textvar{f}$ to the cell because the current concentration of is low. If $[\textvar{A}]$ is equal to 0.8, then we will only add $0.2 \cdot \textvar{f}$ to the current concentration of \textvar{A} particles.
...
\noindent Applying these reaction-diffusion computations over all cells in parallel and over many generations constitutes a cellular automaton called the \textdef{Gray-Scott model}{Gray-Scott model}{a cellular automaton model generating Turing patterns}.
...
Now we will consider the three reactions. The feed reaction will cause the cell's concentration of \textvar{A} to increase by $(1 - [A]) \cdot f = (1-0.7) \cdot 0.3 = 0.09$. The kill reaction will cause its concentration of \textvar{B} to decrease by $k \cdot [B] = 0.4 \cdot 0.5 = 0.2$. And the reproduction reaction will mean that the concentration of \textvar{A} decreases by $[A] \cdot [B]^2 = 0.7 \cdot 0.5^2 = 0.175$, with the concentration of \textvar{B} increasing by the same amount.

As a result, we update the concentrations of \textvar{A} and \textvar{B} to the values $([A]_{\text{new}}, [B]_{\text{new}})$ in the next time step according to the equations above:
\begin{align*}
...
In \autoref{fig:gray-scott_movie_first_frame}, we use the parameters  $f = 0.034$ and $k = 0.100$. We use a comparable initial configuration of the automaton as in the diffusion example, in which a cluster of \textvar{B} particles are found in a board full of \textvar{A} particles.
...
\end{tabular}
\caption{An animation of the Gray-Scott model for \textvar{f} = 0.034 and \textvar{k} = 0.1, in which a single cluster of \textvar{B} particles expands outward into clear stripes.}
\label{fig:gray-scott_movie_first_frame}
\end{figure}

If we expand the size of the simulation and add multiple clusters of \textvar{B} particles to the automaton, then the patterns become more complex as waves of \textvar{B} particles collide (\autoref{fig:gray-scott_multiple_predators_first_frame}). If we keep \textvar{f} constant and increase \textvar{k} slightly to 0.102, then the patterns change significantly into spots (\autoref{fig:gray-scott_f38_k102_first_frame}). And if we make the \textvar{A} particles a little happier as well, increasing  \textvar{f} to 0.044 and keeping \textvar{k} at 0.102, then we have a different striped pattern (\autoref{fig:gray-scott_f44_k102_first_frame}).\\
...
\end{tabular}
\caption{An expanded version of the animation from \autoref{fig:gray-scott_movie_first_frame} containing several initial clusters of \textvar{B} particles. The same pattern is produced, albeit over a larger region.}
...
\end{tabular}
\caption{A Gray-Scott simulation with \textvar{f} = 0.044 and \textvar{k} = 0.102, showing a different striped pattern than \autoref{fig:gray-scott_multiple_predators_first_frame}.}
...
Later in this book, we will see an example of a biological system that is the opposite of fine-tuned. In a \textdef{robust}{robustness}{the ability of a system to retain a desired behavior despite perturbations such as changes to parameters} system, perturbations such as parameter variations do not lead to substantive changes in the ultimate behavior of the system. Robustness is vital for processes, like your heartbeat, that must be resilient to small environmental changes.
...
In \autoref{fig:two_particle_concentration_diffusion}, we showed a grid of cells containing concentrations of two particles \textvar{A} and \textvar{B} that start with concentration equal to 1 in the central square and diffuse according to the rates $d_A = 0.2$ and $d_B = 0.1$.\\

\begin{exercise}[%
Instead of solely diffusing the particles, update the original grid (in which \textvar{A} and \textvar{B} have concentration equal to 1 in the central cell) for two time steps according to the Gray-Scott model. Use $f = 0.03$ and $k = 0.1$.
]\end{exercise}

\phantomsection
\subsection{Changing the predator-prey reaction}

Both our particle simulator and the Gray-Scott model used a reaction $A + 2B \rightarrow 3B$ to represent a predator-prey dynamics of sorts. But there is no reason \textit{a priori} why we would have used this reaction; instead, we could have modeled the simpler reaction $A + B \rightarrow 2B$, in which a predator molecule collides with a prey molecule, and the prey molecule changes into a predator. Because this reaction only requires the collision of two particles, it would be more frequent than the reaction $A + 2B \rightarrow 3B$ if all else is equal.\\

\begin{exercise}[%
Adapt the particle-based simulation that we introduced earlier in this chapter to replace the reaction $A + 2B \rightarrow 3B$ with $A + B \rightarrow 2B$. Play around with the system's reaction rate parameters; are you still able to generate Turing patterns?
]\end{exercise}
...
\phantomsection
\subsection{Adjusting Gray-Scott parameters}
...
\begin{exercise}[%
Adapt the simulation of the Gray-Scott model to handle each of these additional two cases. Do you see the same patterns arise? (See the \href{https://biologicalmodeling.org/prologue/exercises}{online version} of this exercise for details.)
]\end{exercise}
