...
Our idea is to grid off two-dimensional space into blocks and store only the \textit{concentration} of each type of particle found inside the block. To simplify things further, we assume that the concentration of a particle in each block is represented by a decimal number (perhaps representing a percentage concentration) as opposed to counting individual particles.
...
\begin{note}[%
The sum of the values in each grid in \autoref{fig:initial_A_concentration_one_time_step} is equal to 1, which ensures the conservation of total mass in the system.
]\end{note}
