...
We denote the receptor molecule by $T$, the ligand molecule by $L$, and the bound complex as $TL$. The reversible reaction is $T + L \longleftrightarrow TL$ and consists of two reactions. The \textdef{forward reaction}{forward reaction}{FILL IN} is $T + L \rightarrow TL$, which takes place at some rate $k_\text{bind}$, and the \textdef{reverse reaction}{reverse reaction}{FILL IN} is $TL \rightarrow T + L$, which takes place at some rate $k_\text{dissociate}$.
...
\begin{qbox}[
What do you think will happen to the steady state concentration of $L$ if the initial concentration ($l_0$) increases or decreases? What if the dissociation rate ($k_\text{dissociate}$) increases or decreases?  Confirm your prediction by changing the parameters above and solving the quadratic formula for $[L]$.
]\end{qbox}
...
\begin{qbox}[
Use a similar argument to show that the units of the dissociation constant $k_\text{dissociate}$ should be $s^{-1}$.
]\end{qbox}
