...
\begin{qbox}[%
We can simulate a bacterium traveling \textit{down} an attractant gradient if $k$, the parameter representing the steepness of the gradient, is negative. In this case, what do you think the plot of phosphorylated CheY over time will look like?
]\end{qbox}
...
We then iterate the two steps of tumbling and running until the total time allocated for the simulation has elapsed.\tutorial[https://biologicalmodeling.org/chemotaxis/tutorial_purerandom]
...
Pointer to traveling down an attractant gradient.
