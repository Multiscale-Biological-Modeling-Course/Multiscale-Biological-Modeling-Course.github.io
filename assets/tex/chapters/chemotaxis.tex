...
In the previous two chapters, we have harnessed the power of randomness to answer practical questions. Munroe's approach exemplifies a \textdef{randomized algorithm}{random algorithm}{a method that uses randomness to solve (or approximately solve) a problem}, or a method that uses randomness to solve a problem. In fact, his algorithm is inspired by nature; Munroe calls the approach ``be an ant'' because it mimics how ants explore their environment for resources. In this chapter, we will see that the lost immortals' algorithm is also similar to the method of exploration taken by a much smaller organism: our old friend \textit{E.~coli}.

Like other prokaryotes, \textit{E.~coli} is tiny, with a rod-shaped body that is 2 µm long and 0.25 to 1 µm wide. In exploring a vast world with sparse resources, \textit{E.~coli} finds itself in a situation comparable to that of the lost immortals.
...
\centering
\mySfFamily
\includegraphics[width = 0.87\textwidth]{../images_CMYK/chemotaxis_tutorial4_ssa_vscode}
...
\centering
\mySfFamily
\includegraphics[width = 0.87\textwidth]{../images_CMYK/chemotaxisphosnew}
...
\phantomsection
\subsection{Bacterial tumbling is constant for different attractant concentrations}
...
\end{tabular}
\caption{Molecular concentrations over time (in seconds) in a chemotaxis simulation for a variety of initial attractant ligand particle numbers: 10,000 (top left), 1,000,000 (top right), and 100,000,000 (bottom). The concentration of CheY, a proxy for tumbling frequency, is shown in orange.}
