...
When we multiply the binding rate $k_\text{bind}$ by the concentrations $[L]$ and $[T]$, the resulting unit should be in $\text{molecules}/\text{µm}^3$ per second, which corresponds to the rate at which the concentration of $LT$ complexes is increasing. If we let \textvar{y} denote the unknown units of $k_\text{bind}$, then
...
As a result, this reaction is shown below. We name our rule specifying the ligand-receptor reaction \texttt{LR}.\tutorial[https://biologicalmodeling.org/chemotaxis/tutorial\_lr]\\
