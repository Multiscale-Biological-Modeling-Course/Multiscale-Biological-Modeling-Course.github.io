...
In the case of chemotaxis, \textit{E. coli} has receptor proteins that detect attractants such as glucose by binding to and forming a complex with these attractant \textdef{ligands}{ligands}{FILL IN}. The cell also contains receptors to detect repellents, but in this module, we will focus primarily on attractants. As shown in the figure below, ligand binding forms a cascade of internal reactions that causes a change in the rotation of one or more flagella.
...
In general, a quadratic equation has the form $a \cdot x^2 + b \cdot x + c = 0$ for constants $a$, $b$, and $c$ and a single variable $c$. In our case, $x$ = $\text{[}LT\text{]}$, and the constants are $a = k_{bind}$, $b = - (k_{bind} \times l_0 + k_{bind} · t_0 + k_{dissociate})$, and $c = k_{bind} \times l_0 \times t_0$.
...
\begin{itemize}
 ...
 \item $b = - (k_{bind} \times l_0 + k_{bind} \times t_0 + k_{dissociate}) = -283.2$
 \item $c = k_{bind} · l_0 · t_0 = 1022000$
\end{itemize}
...
\begin{itemize}
\item $\text{[}LT\text{]} = 4,793 ((molecules\mathbin{/}µm^3)$
\item $\text{[}L\text{]} = 5,207 ((molecules\mathbin{/}µm^3)$
\item $\text{[}T\text{]} = 2,207 ((molecules\mathbin{/}µm^3)$
\end{itemize}
...
For many cellular responses, including bacterial chemotaxis, a sequence of phosphorylation and dephosphorylation events called a \textdef{phosphorylation cascade}{phosphorylation cascade}{FILL IN} serves to transmit information within the cell about the amount of ligand binding being detected on the cell's exterior. In this lesson, we discuss how this cascade of chemical reactions leads to a change in bacterial movement.
...
In the previous lesson, we very briefly introduced BioNetGen as a way to convert reactions into a software package applying the Gillespie algorithm. However, BioNetGen is useful not only for running particle-free simulations, but also because it implements its own language for \textdef{rule-based modeling}{rule-based modeling}{FILL IN}.
