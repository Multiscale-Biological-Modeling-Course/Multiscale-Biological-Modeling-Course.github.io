...
\phantomsection
\subsection{Pitfalls of neural networks, Alphafold, and final reflections}

At the time of writing, neural networks are wildly popular. However, they are not a cure-all, as many problems exist for which they are struggling to make progress. Furthermore, neural networks come with their own issues. Because we have so much freedom for how the neural network is formed, it is challenging to know \textit{a priori} how to design an ``architecture'' for how the neurons should be connected to each other for a given problem.

Once we have decided on an architecture, the neural network has so many bias and weight parameters that even with access to a supercomputer, it may be difficult to find parameters that perform well on the training set. ALPHAFOLD parameters

Furthermore, even if a model has a low mean RMSD for the training set, then it may perform horribly on the test set, which is yet another instance of the curse of dimensionality. (The former situation is called ``underfitting'' and the latter is called ``overfitting''.)
...
AT SOME POINT, CALL BACK TO ALPHAFOLD (21 million parameters). Critical idea here is that a neural network can be thought of a big, bulky thing that takes some things and produces others, a complicated function. We already know how to vectorize structures, so we could train a neural network to take sequences (encoded as vectors) and produce their shape vectors. What AlphaFold does is more complicated than this, involving a few different neural networks, but the idea is still the same.
