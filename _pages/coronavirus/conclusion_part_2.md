...
The equilibrium position of node *i* is represented by the vector *R*<sub>*i*</sub><sup>0</sup>, and its position at a given point in time is denoted by the (variable) vector Δ*R*<sub>*i*</sub>. The distance between node *i* and node *j* at equilibrium is denoted by the vector $$ R_{ij}^0 $$, which is equal to $$ R_j^0 - R_i^0 $$ (as illustrated in the figure below); at a given point in time, this distance becomes the vector $$ R_{ij} $$.
